...
public class FacilityServiceImpl implements FacilityService{
	...
	private FeedBarrelRepository feedBarrelRepository;
	...
	public boolean registerFacility(Facility facility,
		...
		try {
			...
			tempFacility = facilityRepository.selectFacility(tempFacility);
			
			if (tempFacility != null) {
				int count = facilityRepository.deleteFacility(tempFacility);
			}
			
			result = facilityRepository.insertFacility(facility);
			
			Facility facility_ = facilityRepository.selectFacility(facility);		//시설물을 미리 등록한후 시설물의 일련번호를 가져와야 사료통과 물통, 담당자를 추가 할 수 있다. (FK)
																		//따라서 시설물의 정보를 다시 가져옴
				...
				if (feedBarrelList != null) {
					for (int i = 0 ; i < feedBarrelList.size() ; i++ ) {
						...
						if (feedBarrel != null) {	//사료통 설렉트 박스에 값이 없음이 아닌경우만 insert
							...
							feedBarrelRepository.insertFeedBarrel(feedBarrel);
						}
					}
				}
				//물통 추가
	...
	public List<Facility> getFacilityList(String term) {
		...
		try {
			facilityList = facilityRepository.selectFacilityList(term);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return facilityList;
	...
	public Map<String, Object> getFacility(Facility facility) { //시설물 no가 반드시 포함되어야함
		...
		//조회한 시설물에 담당자가 존재하는지? (시설물id를 이용)
			...
			try {
				...
				facilityMap.put("facility", facilityRepository.selectFacility(facility));	//시설물 (상세)조회
...
//						memberList.add(memberMapper.selectMember(member));
				...
				if (facilityMap.get("facility") != null) {
					if (((Facility)(facilityMap.get("facility"))).getType().equals("F")) { 			//급식소라면 물통 사료통도 조회함
						facilityMap.put("feedBarrelList", feedBarrelRepository.selectFeedBarrelList(feedBarrel));
	...
	public boolean editFacility(Facility facility,
		...
		try {
			result = facilityRepository.updateFacility(facility);
			...
			List<FeedBarrel> feedBarrelListDb = feedBarrelRepository.selectFeedBarrelList(feedBarrelDb);
			...
			for (FeedBarrel feedBarrelDb_ : feedBarrelListDb) {
				feedBarrelRepository.deleteFeedBarrel(feedBarrelDb_);
			}
			...
			if (feedBarrelList != null) {
				for (FeedBarrel feedBarrel_ : feedBarrelList) {
					...
					feedBarrelRepository.insertFeedBarrel(feedBarrel_);
				}
			}
	...
	public boolean deleteFacility(Facility facility) {
		...
		try {
			...
			if (facility != null) {
				result = facilityRepository.deleteFacility(facility);
				if (facility.getType().equals("F"))	 {
					FeedBarrel feedBarrel = new FeedBarrel();
					feedBarrel.setFacilityNo(facility.getNo());
					feedBarrelRepository.deleteFeedBarrel(feedBarrel);
	...
	public void editFeedBarrelList(Facility facility, List<FeedBarrel> feedBarrelList) {
		try {
			for (int i = 0 ; i < feedBarrelList.size() ; i++) {
	            ...
	            feedBarrel.setFacilityNo(facilityRepository.selectFacility(facility).getNo());
	            ...
	            originalBarrel = feedBarrelRepository.selectFeedBarrel(originalBarrel);
				...
				feedBarrelRepository.updateFeedBarrel(feedBarrel);
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public void editWaterBarrelList(Facility facility, List<WaterBarrel> waterBarrelList) {
		try {
			for (int i = 0 ; i < waterBarrelList.size() ; i++) {
				...
				waterBarrel.setFacilityNo(facilityRepository.selectFacility(facility).getNo());
...
//		managementFacility.setMemberNo(member.getNo());
	...
	public void setTempFacilityUid(Facility facility) {
		try {
			...
			if (facilityRepository.selectFacility(facility) == null) {
				facility.setNo(-1);
				facilityRepository.insertTempFacility(facility);
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public List<FeedBarrel> getFeedBarrelList(int facilityNo) {
		try {
			...
			if (facilityRepository.selectFacility(facility).getType().equals("F")) {
				FeedBarrel feedBarrel = new FeedBarrel();
				feedBarrel.setFacilityNo(facilityNo);
				System.out.println(feedBarrel.getFacilityNo());
				return feedBarrelRepository.selectFeedBarrelList(feedBarrel);
			}
